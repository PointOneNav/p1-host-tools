...
from p1_runner.log_manager import DeviceType, LogManager
...
class InterfaceConfig(NamedTuple):
    # The name of the interface (uart1 or uart2)
    name: str
    # The serial port to connect to.
    port: str
    # A list of tests to run ("version", "uart_ids", "expected_storage", "reboot",
    # "watchdog_fault", "msg_rates", "import_config", "set_config", "save_config").
    tests: List[str]


class TestConfig(NamedTuple):
    # The interfaces to run tests on.
    interfaces: List[InterfaceConfig]
    baud_rate: int = 460800
    # A regex to match when checking the FusionEngine version.
    expected_version: Optional[str] = None
    # A file containing a configuration saved from the device. If specified a
    # test will check if the device's active configuration matches the values in
    # this save.
    ...
    # Whether to raise an error if there are unsaved configuration changes on
    # the device.
    allow_unsaved_config_changes: bool = False
...
DEFAULT_CONFIG = TestConfig(
    interfaces=[
        InterfaceConfig(
            name='uart1', port=find_serial_device(port_type=PortType.ENHANCED),
            tests=['version', 'uart_ids', 'expected_storage', 'msg_rates', 'import_config']),
        InterfaceConfig(
            name='uart2', port=find_serial_device(port_type=PortType.STANDARD),
            tests=['version', 'uart_ids', 'expected_storage', 'msg_rates', 'set_config', 'import_config',
                   'set_config_exhaustive', 'reboot', 'watchdog_fault', 'expected_storage'])],
    expected_version=r'lg69t-.+',)
...
def interface_name_to_idx(name: str) -> Optional[int]:
    if name == 'uart1':
        return 1
    elif name == 'uart2':
        return 2
...
def test_version(state: TestState) -> None:
    """!
    ...
    if not isinstance(resp, VersionInfoMessage):
        ...
        if state.config.expected_version is not None:
            m = re.match(state.config.expected_version, version_str)
            if m is None:
                logger.error('FusionEngine version response %s did not match expected version %s.',
                             version_str, state.config.expected_version)
                state.passed = False

        if version_str.startswith('lg69t-ap'):
            device_type = DeviceType.LG69T_AP
        elif version_str.startswith('lg69t-am'):
            device_type = DeviceType.LG69T_AM
        elif version_str.startswith('lg69t-ah'):
            device_type = DeviceType.LG69T_AH
        else:
            device_type = DeviceType.UNKNOWN
            logger.error('Could not deduce device type from version string.')
            state.passed = False

        state.test_logger.update_manifest([
...
def test_uart_ids(state: TestState) -> None:
    """!
    ...
    if response_interface is None:
        ...
        if response_interface != InterfaceID(TransportType.SERIAL, state.interface_idx):
            logger.error('Response had unexpected interface ID %s.', response_interface)
            state.passed = False
...
def test_expected_storage(state: TestState) -> None:
    ...
    if saved_storage is None:
        ...
        if not state.config.allow_unsaved_config_changes and active_config:
            if saved_config != active_config:
                logger.error('Active configuration differs from saved configuration.')
                state.passed = False

        if state.config.expect_calibration_done:
        ...
        if state.config.expected_config_save is not None:
            logger.debug("Checking if saved config matched expected values.")
            expected_storage = load_saved_data(state.config.expected_config_save, [DataType.USER_CONFIG])
...
def test_msg_rates(state: TestState) -> None:
    ...
    args = Namespace(param=f'{state.interface_name}_message_rate', protocol='all',
    ...
    args = Namespace(param=f'{state.interface_name}_diagnostics_enabled', enabled=False, save=False)
    if not apply_config(state.device_interface, args):
        logger.error('Request failed.')
        state.passed = False
        check_exit(state)
    ...
    args = Namespace(param=f'{state.interface_name}_message_rate', protocol='fe',
    ...
    args = Namespace(param=f'{state.interface_name}_message_rate', protocol='nmea',
    ...
    if not state.device_interface.wait_for_message('$GNGGA'):
...
def load_config(config_file: str) -> TestConfig:
    ...
    with open(config_file) as fd:
        data = json.load(fd)
    interfaces = [InterfaceConfig(**i) for i in data['interfaces']]
    del data['interfaces']

    return TestConfig(**data, interfaces=interfaces)
...

{
    "interfaces": [
        {"name": uart1 or uart2,
         ...
         "tests": subset of tests to run ["version", "uart_ids", "expected_storage", "reboot", "watchdog_fault", "msg_rates", "set_config", "import_config", "save_config"]
         },
         [Optional] Other interfaces
    ],
    "expected_version": [Optional] regex to match with the version reported by the device,
    "expected_config_save": [Optional] configuration backup file generated from config_tool,
    "expect_calibration_done": [Optional] whether to check for a completed calibration on the device,
    "allow_unsaved_config_changes": [Optional] whether to check if the device had unsaved modifications to its configuration
}
...
''' % {'command': execute_command})
    ...
    parser.add_argument(
        ...
        help="An ID string used to identify this device in the recorded log data. Set to the value of the P1_DEVICE_ID "
             ...
             DEFAULT_DEVICE_ID)
    parser.add_argument(
        '-t', '--test-configuration', default=None,
    ...
    if args.device_id is None:
        device_id = os.environ.get('P1_DEVICE_ID', DEFAULT_DEVICE_ID)
    else:
        device_id = args.device_id

    module = sys.modules[__name__]

    if args.test_configuration is None:
        config = DEFAULT_CONFIG
    else:
        config = load_config(args.test_configuration)

    tests_passed = True
    for interface_config in config.interfaces:
        set_logging_indent(0)
        logger.info('Running tests on interface %s (%s).' % (interface_config.name, interface_config.port))
        interface_idx = interface_name_to_idx(interface_config.name)
        if interface_idx is None:
            logger.error('Invalid interface %s (%s).' % (interface_config.name, interface_config.port))
            if args.continue_on_failures:
                continue
            else:
                exit(2)
        serial_port = Serial(port=interface_config.port, baudrate=config.baud_rate, timeout=SERIAL_TIMEOUT)
        logger_manager = LogManager(device_id)
        logger_manager.start()
        data_source = SerialDataSource(serial_port, rx_log=logger_manager)
        data_source.start_read_thread()
        interface = DeviceInterface(data_source)

        state = TestState(
            device_interface=interface, test_logger=logger_manager, interface_name=interface_config.name,
            interface_idx=interface_idx, config=config, fail_fast=not args.continue_on_failures)

        time.sleep(0.2)
        for test_name in interface_config.tests:
